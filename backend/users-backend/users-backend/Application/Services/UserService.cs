    ...
    public override UserDto Update(UserDto userDto)
    {
        ...
        if (!user.RowVersion.SequenceEqual(userDto.RowVersion))
        {
            throw new ConcurrencyException(
                "El usuario ya fue actualizado por otro. Actualiza la información antes de continuar.");
        }
        ...
        {
            // Envuelve y lanza una excepción personalizada
            throw new ConcurrencyException(
                "El usuario ya fue actualizado por otro. Actualiza la información antes de continuar.", ex);
        }
    }
}